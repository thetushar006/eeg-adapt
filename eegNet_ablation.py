...
class EEGNetv4(BaseModel):
    ...
    def create_network(self):
        ...
        if self.n_ablation > 0:
            for i in range(self.n_ablation):
                # https://discuss.pytorch.org/t/how-to-modify-a-conv2d-to-depthwise-separable-convolution/15843/7
                model.add_module(
                    f"conv_separable_depth_{i}",
                    nn.Conv2d(
                        self.F1 * self.D,
                        self.F1 * self.D,
                        (1, 16),
                ...
                model.add_module(f"elu_2_{i}", Expression(elu))
                model.add_module(
                    f"pool_2_{i}", pool_class(kernel_size=(1, 4), stride=(1, 4))
                )
                model.add_module(f"drop_2_{i}", nn.Dropout(p=self.drop_prob))
